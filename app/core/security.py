...
def determine_token_type(token_type: TokenType) -> tuple[str, timedelta]:
    ...
    if token_type == TokenType.ACCESS:
        return settings.access_secret_key, timedelta(
            minutes=settings.access_token_expire_minutes
        )
    elif token_type == TokenType.REFRESH:
        return settings.refresh_secret_key, timedelta(
            days=settings.refresh_token_expire_days
        )
    elif token_type == TokenType.VERIFICATION:
        return settings.verification_secret_key, timedelta(
            minutes=settings.verification_token_expire_minutes
        )
    elif token_type == TokenType.PASSWORD_RESET:
        return settings.password_reset_secret_key, timedelta(
            minutes=settings.password_reset_token_expire_minutes
        )


def to_encode(
    user_data: User, token_type: TokenType, expire: datetime
) -> TokenPayload | VerificationToken:
    ...
    if token_type in (TokenType.VERIFICATION, TokenType.PASSWORD_RESET):
        return VerificationToken(
            email=user_data.email, scope=token_type.value, exp=expire
        )
    else:
        return TokenPayload(
...
def create_token(user_data: User, token_type: TokenType) -> str:
    ...
    encoded_jwt = jwt.encode(
        to_encode(user_data, token_type, expire).model_dump(),
        secret_key,
